...
after 'ci:size_option',
  ...
  @offers = []
  @offer_settings = []
  I18n.t('seeds.products_with_color').each do |product_seed|
    name = product_seed[:name]
    product = Spree::Product.find_or_initialize_by(
      name: name,
      description: 'This is a test product description!',
      available_on: Time.zone.now,
      option_types: [Spree::OptionType.find_by(name: 'Size'), Spree::OptionType.find_by(name: 'Color')]
    )
    ...
    I18n.t("seeds.variants.#{name.parameterize}").each do |variant_data|
      ...
      variant.option_values << Spree::OptionValue.find_by!(name: variant_data[:color])
    end
