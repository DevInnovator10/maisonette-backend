...
var ShipmentEditView = Backbone.View.extend({
  initialize: function(){
    ...
    this.$(".edit-giftwrap").each(function(el){
      new Spree.Views.Order.ShipmentGiftwrap({
        el: this,
        model: shipment
      });
    });
...
var OrderEditView = Backbone.View.extend({
  initialize: function(){
    this.order_number = this.model.get('number');
    order = this.model;
    this.$(".js-edit-store-credits").each(function(el){
      new Spree.Views.Order.EditStoreCredits({
        el: this,
        model: order
      });
    });
    this.$(".js-gift-details").each(function(el){
      new Spree.Views.Order.Gift({
        el: this,
        model: order
      });
    });
  ...
  if($('.js-shipment-edit [data-order-number]').length) {
    ...
    var order = Spree.Models.Order.fetch(orderNumber, {
      success: function(order){
        $(".js-order-edit").each(function(){
          new OrderEditView({ el: this, model: order });
        });
