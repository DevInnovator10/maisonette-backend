...
json.cache! [I18n.locale, shipment] do # rubocop:disable Metrics/BlockLength
  ...
  json.giftwrappable(shipment.giftwrappable?)
  json.stock_location_id(shipment.stock_location_id)
  json.giftwrap do
    if shipment.giftwrap.present?
      json.partial!('maisonette/api/giftwraps/small', giftwrap: shipment.giftwrap)
    else
      json.nil!
    end
  end
  json.shipping_rates(shipment.shipping_rates) do |shipping_rate|
    json.partial!('spree/api/shipping_rates/small', shipping_rate: shipping_rate)
  end
  json.selected_shipping_rate do
    if shipment.selected_shipping_rate
      json.partial!('spree/api/shipping_rates/big', shipping_rate: shipment.selected_shipping_rate)
    else
      json.nil!
    end
  end
